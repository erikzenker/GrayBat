...
namespace graybat {
    ...
    template <typename T_Data, typename Op>
    auto Cage<T_CommunicationPolicy, T_GraphPolicy>::
    reduce(const Vertex & rootVertex, const Vertex & srcVertex, Op op, const std::vector<T_Data> sendData, std::vector<T_Data>& recvData)
    -> void
    {
    ...
    template <typename T_Send, typename T_Recv>
    auto Cage<T_CommunicationPolicy, T_GraphPolicy>::
    gather(const Vertex & rootVertex, const Vertex & srcVertex, const T_Send sendData, T_Recv& recvData, const bool reorder)
    -> void
    {
